...
b = ones(v,1);
    ...
    for e =1:size(Htmp,2)
        ...
        if flag
            ...
            r_e = [Htmp(v+1,e)-Htmp(1:v,e)'*inv(B)'*O(v+1,m+1:m+v)' Htmp(1:v,e)'*inv(B)'];
...
%                     H_test = H;
%                     H_test(:,j+m) = Htmp(:,e);
%                     old_cost = ones(1,v)*(inv(H(1:v,m+1:m+v))*H(v+1,m+1:m+v)');
%                     new_cost = ones(1,v)*(inv(H_test(1:v,m+1:m+v))*H_test(v+1,m+1:m+v)');
